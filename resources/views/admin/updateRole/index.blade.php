...
@section('title', 'Update Role of Users List')
@section('pageTitle')
    <h1 class="font-bold text-2xl ml-3">All of the users List</h1>
@endsection
...
@section('contents')
    <div style="margin:.5vw">
        ...
        @endif
        <table id="updateRole-table" class="display cell-border" style="width:100%">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Name</th>
                ...
                @if ($users->count() > 0)
                    @foreach ($users as $rs)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('admin.updateRole.update', ['id' => $rs->id]) }}" method="POST">
                                    @csrf
                                    <!-- Provide a select input for updating the role -->
                                    <select name="type" id="type" class="rounded">
                                        <option value="admin" {{ $rs->type == 'admin' ? 'selected' : '' }}>Admin</option>
                                        <option value="manager" {{ $rs->type == 'manager' ? 'selected' : '' }}>Manager
                                        </option>
                                        <option value="tourGuide" {{ $rs->type == 'tourGuide' ? 'selected' : '' }}>
                                            TourGuide</option>
                                        <option value="user" {{ $rs->type == 'user' ? 'selected' : '' }}>User</option>
                                    </select>
                                    <!-- Update button -->
                            ...
                            <td>
                                <div class="h-14 pt-5">
                                    <form action="{{ route('admin.updateRole.destroy', $rs->id) }}" method="POST"
...
@push('scripts')
    ...
    <script>
        let table = new DataTable('#updateRole-table', {
            layout: {
                top1Start: {
                    buttons: [{
                            extend: 'copy',
